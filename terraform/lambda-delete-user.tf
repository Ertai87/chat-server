resource "aws_lambda_function" "lambda_delete_user_function" {
  ...
  role = aws_iam_role.lambda_delete_user_execution_role.arn
  runtime = "nodejs12.x"
}

resource "aws_iam_role" "lambda_delete_user_execution_role" {
...
resource "aws_iam_role_policy_attachment" "delete_user_sqs_full_access" {
  ...
  role = aws_iam_role.lambda_delete_user_execution_role.name
}
...
resource "aws_iam_role_policy_attachment" "delete_user_cloudwatch_logs" {
